...
public class SeatCalendar {
	...
	LocalDate currentDate = LocalDate.of(2020, 2, 5);
	private HashMap<Integer, TimeSlot> timeSlots;
	
	// constructors
	public SeatCalendar(LocalTime start, LocalTime end) {
		availableHoursStart = start;
		availableHoursEnd = end;
		createTimeSlots( start, end.minusMinutes(TimeSlot.durationOfMeal - 1));
	}
	...
	private void createTimeSlots( LocalTime start, LocalTime end)
	{
		...
		for( LocalTime i = start; i.isBefore(end); i = i.plusMinutes(1)) // 1 represent intervals, we can change if needed
		{
